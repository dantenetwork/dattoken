...
const web3 = new Web3('http://192.168.1.49:6789');
...
const PRIVATE_KEY = "0x720fd94b1093d6fd58704491510f6e60869f721f7a40087d5d05f303a4bea403"; /// owner private key
...
// 合约的执行接口
describe('Token Action Test Case', () => {
  // 测试 owner 账户转出 DAT
  describe('Transfer', () => {
    it("Transaction status should equal true", async function () {
      this.timeout(0);
      const signTx = await signTransaction("transfer", PRIVATE_KEY, [nonOwnerAddress, ONE_TOKEN]);
  ...
  describe('Mint', () => {
    it("Transaction status should equal true", async function () {
      this.timeout(0);
      const signTx = await signTransaction("mint", PRIVATE_KEY, [ownerAddress, ONE_TOKEN]);
  ...
  describe('Burn', () => {
    it("Transaction status should equal true", async function () {
      this.timeout(0);
      const signTx = await signTransaction("burn", PRIVATE_KEY, [ownerAddress, ONE_TOKEN]);
      ...
      try {
        await web3.platon.sendSignedTransaction(signTx.rawTransaction,
          async (err, transactionId) => {
            // 通过交易 id 查询交易 status 值，如果为 false 说明执行失败
            web3.platon.getTransactionReceipt(transactionId)
              .then((result) => {
